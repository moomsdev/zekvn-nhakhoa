...
class WmuHelper implements WmuConstants {
    ...
    public function getAttachedVideos($attachIds, $currentUser = null, $size = "thumbnail") {
        ...
        if ($attachIds) {
            ...
            foreach ($attachments as $attachment) {
                $attachmentId = WpdiscuzHelperUpload::encrypt($attachment->ID);
                ...
                if (preg_match("#^video#is", $attachment->post_mime_type)) {
                    if ($this->options->wmuIsHtml5Video && in_array($attachment->post_mime_type, $this->html5VideoTypes)) {
                        ...
                        $video .= "<div class='wmu-attachment wmu-attachment-$attachmentId wmu-video'>";
                        ...
                        $video .= "<div class='wmu-attachment wmu-attachment-$attachmentId'>";
                    ...
                    if ($this->options->wmuIsHtml5Audio && in_array($attachment->post_mime_type, $this->html5AudioTypes)) {
                        $video .= "<div class='wmu-attachment wmu-attachment-$attachmentId wmu-audio'>";
    ...
    public function getAttachedFiles($attachIds, $currentUser = null, $size = "thumbnail") {
        ...
        if ($attachIds) {
            ...
            foreach ($attachments as $attachment) {
                ...
                $filename = $this->getFileName($attachment);
                $files .= "<div class='wmu-attachment wmu-attachment-$attachmentId'>";
    ...
    public function frontendCommentsFilterHtml($currentUser, $wpdOptions) {
        if (!$wpdOptions->wp["isPaginate"]) {
            $buttonMediaFilterText = $this->options->wmuPhrases["wmuPhraseButtonMediaFilter"] ? esc_html($this->options->wmuPhrases["wmuPhraseButtonMediaFilter"]) : esc_html__("Comments with attachments", "wpdiscuz-media-uploader");
            ?>
            <div class="wpd-filter wpdf-attachments wpd_not_clicked" data-filter-type="attachments" wpd-tooltip="<?php echo $buttonMediaFilterText; ?>">
